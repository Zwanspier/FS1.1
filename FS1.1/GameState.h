...
enum GameState {
    ...
    PRELEVEL1,    // Pre-level screen for level 1 (scrolling text game)
                  ...
                  // Transition: To PLAYING (start Level 1)
    ...
    PRELEVEL2,    // Pre-level screen for level 2 (maze navigation)
                  ...
                  // Transition: To PLAYING2 (start Level 2)
    ...
    PRELEVEL3,    // Pre-level screen for level 3 (driving game)
                  ...
                  // Transition: To PLAYING3 (start Level 3)
    ...
    PLAYING,      // Level 1: Scrolling Text Game
                  ...
                  // Transition: To PRELEVEL2 (advance to next level)
    ...
    PLAYING2,     // Level 2: Maze Navigation Game  
                  ...
                  // Transition: To PRELEVEL3 (advance to next level)
    ...
    PLAYING3,     // Level 3: Driving/Racing Game
                  ...
                  // Transition: Back to MENU (completion or exit)
...
// 
// State Flow Overview:
// MENU ? PRELEVEL1 ? PLAYING ? PRELEVEL2 ? PLAYING2 ? PRELEVEL3 ? PLAYING3 ? MENU
//   ?                                                                           ?
// SETTINGS ???????????????????????????????????????????????????????????????????????
//
